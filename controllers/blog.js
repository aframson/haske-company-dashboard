...
const addgiftcard = async (title, description,filename, status, author, downloadURL, setMsg, errMsg, setLoading, setProgress) => {
    setLoading(true)
    try {
...
export const handleUpload = async (title, description,filename, status, author, setLoading, imageLocation, setProgress, setMsg, errMsg) => {
    // get file name from file
    setLoading(true)
    const storage = getStorage();
    ...
    uploadTask.on('state_changed', (snapshot) => {
        ...
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
            console.log('File available at', downloadURL);
            addgiftcard(title, description,uniquename ,status, author, downloadURL, setMsg, errMsg, setLoading, setProgress);
        });
        setLoading(false)
    });
...
export async function fetchBlogs(setGiftCard, setLoading, setAdd, errMsg, setIsProduct, setLoad, type) {
    setLoading(true)
    try {
