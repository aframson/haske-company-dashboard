...
const TableName = "giftcards"
const fileLocation = 'giftcard/'
...
const addgiftcard = async (title, description,filename, status, dominationsData, downloadURL, setMsg, errMsg, setLoading, setProgress) => {
    setLoading(true)
    try {
...
export const handleUpload = async (title, description,filename, status, dominationsData, setLoading, imageLocation, setProgress, setMsg, errMsg) => {
    // get file name from file
    setLoading(true)
    const storage = getStorage();
    ...
    uploadTask.on('state_changed', (snapshot) => {
        ...
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
            console.log('File available at', downloadURL);
            addgiftcard(title, description,uniquename,status, dominationsData, downloadURL, setMsg, errMsg, setLoading, setProgress);
        });
        setLoading(false)
    });
...
export async function fetchGiftCards(setGiftCard, setLoading, setAdd, errMsg, setIsProduct, setLoad, type) {
    setLoading(true)
    try {
