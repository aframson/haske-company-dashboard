...
import { fetchDiscounts, delData } from '../controllers/discount'
...
export default function AllDiscounts({ setIsProduct, setLoad, type }) {
  ...
  const Delete = (id, filename, products, errMsg, setMsg, setLoading2, setDiscount, discountData) => {
    setdeId(id)
    if (confirm("Are you sure you want to do that?")) {
      delData(id, filename, products, errMsg, setMsg, setLoading2, setDiscount, discountData)
  ...
  useEffect(() => {
    fetchDiscounts(setDiscount, setLoading, setAdd, errMsg, setIsProduct, setLoad, type)
  }, [setDiscount, setLoading, setAdd, errMsg, setIsProduct, setLoad, type])
    ...
    <div className={styles.tablerow}>
      <div className={styles.tablecontent}>
        <div className={styles.tablehead}>
          <div className={styles.th}>Discount</div>
        ...
        {discountData && discountData.map((item, i) => (
          <div key={i} className={styles.body}>
            ...
            <div onClick={() => Delete(item.id, item.filename, item.products, errMsg, setMsg, setLoading2, setDiscount, discountData)} id={styles.action} className={styles.tb}>
